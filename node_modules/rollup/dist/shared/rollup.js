...
const childNodeKeys = {
    ...
    // TODO: investigate why attributes are used here, we don't have them on ExportNamedDeclaration
...
const nodeConstructorsAsObject = {
  PanicError,
  ParseError,
  ArrayExpression,
  ArrayPattern,
...
class Module {
    ...
    async setSource({ ast, code, customTransformCache, originalCode, originalSourcemap, resolvedIds, sourcemapChain, transformDependencies, transformFiles, ...moduleOptions }) {
        ...
        if (ast) {
            ...
            // console.log(stringifyWithCircularCheck(this.ast.body));
            const { parseAsync } = require('oxc-parser');
            const ret = await parseAsync(code);
            const ast = JSON.parse(ret.program);
            // console.log(ast);
            

            // function convertNode(ast, nodeType, programParent, parentScope) {
            //   const NodeConstructor = nodeConstructorsAsObject[nodeType];
            //   if (!NodeConstructor) {
            //     console.trace();
            //     throw new Error(`Unknown node type: ${nodeType}`);
            //   }
            //   const node = new NodeConstructor(programParent, parentScope);
            //   node.type = nodeType;
            //   node.start = '';
            //   node.end = '';
            //   debugger;
            ...
            function traverseAndConvert(obj, parentNode=programParent) {
              if (obj && typeof obj === 'object') {
                obj.type = obj.type === 'Module' ? parseAst_js.Program : obj.type;
                obj.type = obj.type === 'StaticMemberExpression' ? 'MemberExpression' : obj.type;
                obj.type = obj.type === 'FormalParameter' ? 'Identifier' : obj.type;
                obj.type = obj.type?.endsWith('Literal') ? 'Literal' : obj.type;
                obj.type = obj.type?.endsWith('Identifier') ? 'Identifier' : obj.type;
                if (obj.type === 'FunctionDeclaration') {
                  debugger;
                  obj.params = obj.params.items;
                  debugger;
                }
                ...
                switch (obj.type) {
                  case 'ArrowFunctionExpression':
                    obj.generator = false;
                    // TODO
                    obj.annotationNoSideEffects = false;
                    // {
                    //   const parameters = obj?.params?.items?.map(i => {
                    //     const newNode = new Identifier(obj, scope);
                    //     Object.assign(newNode, i);
                    //     return newNode;
                    //   });
                    //   obj.params = parameters;
                    //   // obj.scope.addParameterVariables(parameters.map(parameter => parameter.declare('parameter', UNKNOWN_EXPRESSION)), parameters[parameters.length - 1] instanceof RestElement);
                    // }
                    break;
                  case 'CallExpression':
                    // TODO
                  ...
                  case 'ImportDeclaration':
                    // @TODO figure out how the import attributes obj
                    // in original rollup looks like
                    // https://oxc-project.github.io/oxc/playground/?code=3YCAAICrgICAgICAgIC0G8rnONK881UOQpORgQm415xeeAogFzxWWCIn6aAWsc7tYJEq%2FZoZSn80xpx8f35hCIA%3D
                    obj.attributes = obj?.withClause?.withEntries?.map(attr => {
                      const newNode = new ImportAttribute(obj, scope);
                      Object.assign(newNode, attr);
                      return newNode;
                    }) ?? [];
                    break;
                  case 'ImportExpression':
                    obj.sourceAstNode = obj.source;
                    // TODO figure out how options obj in original
                    // rollup looks like
                    // https://oxc-project.github.io/oxc/playground/?code=3YCAAICRgICAgICAgIC0G8rnONK8G2ZYxTOGq516SuJx72rLf0%2BggIA%3D
                    obj.options = {};
                    break;
                  case 'Literal':
                    obj.raw = JSON.stringify(obj.value);
                    break;
                  case 'Program':
                    // TODO
                ...
                if (obj.type && nodeConstructorsAsObject[obj.type]) {
                  // This is a node, convert it
                  const NodeConstructor = nodeConstructorsAsObject[obj.type];
                  const newNode = new NodeConstructor(parentNode, scope);
                  newNode.type = obj.type;
                  newNode.start = '';
                  newNode.end = '';
                  Object.assign(newNode, obj);
                  // Recursively convert child nodes
                  Object.keys(obj).forEach(key => {
                    if (typeof obj[key] === 'object') {
                      newNode[key] = traverseAndConvert(obj[key], newNode);
                    }
                  });
                  newNode.initialise();
                  return newNode;
                } else {
                  // This is not a node, but might contain nodes
                  if (Array.isArray(obj)) {
                    return obj.map((e) => traverseAndConvert(e, parentNode));
                  } else {
                    const newObj = {};
                    Object.keys(obj).forEach(key => {
                      newObj[key] = traverseAndConvert(obj[key], parentNode);
                    });
                    return newObj;
                  }
                }
            ...
            const newAst = traverseAndConvert(ast);
            delete newAst.directives;
            delete newAst.sourceType;
            delete newAst.hashbang;
            // TODO: support rollup annotations, e.g. PURE
            newAst.invalidAnnotations = [];
            newAst.parent = programParent;
            // console.log(stringifyWithCircularCheck(newAst));

            // debugger;

            // this.ast = convertProgram(ast, programParent, this.scope);
            this.ast = newAst;
    ...
    addImport(node) {
      ...
      console.log('count:', this.count);
        ...
        for (const specifier of node.specifiers) {
            ...
            console.log(specifier instanceof ImportDefaultSpecifier);
